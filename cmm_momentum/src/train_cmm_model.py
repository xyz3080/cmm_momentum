...
MODEL_CONFIG = config_section("model")
SEED = int(MODEL_CONFIG.get("seed", 42))
INITIAL_TRAIN_MONTHS = int(MODEL_CONFIG.get("initial_train_months", 60))
VAL_FRACTION = float(MODEL_CONFIG.get("val_fraction", 0.30))
TEST_BLOCK_MONTHS = int(MODEL_CONFIG.get("test_block_months", 24))
MIN_TEST_BLOCK_MONTHS = int(MODEL_CONFIG.get("min_test_block_months", 6))
N_EPOCHS = int(MODEL_CONFIG.get("n_epochs", 80))
...
HIDDEN = tuple(int(x) for x in MODEL_CONFIG.get("hidden", [128, 64, 32]))
DROPOUT = float(MODEL_CONFIG.get("dropout", 0.05))
SAVE_VALIDATION_PREDICTIONS = bool(MODEL_CONFIG.get("save_validation_predictions", False))
...
def main() -> None:
    ...
    for fold_cfg in expanding_splits(months):
        ...
        torch.save(
            ...
            fold_model_dir / f"cmm_model_fold_{fold:02d}.pt",
        )
